...
# 📂 Répertoire des scripts et données
BASE_FOLDER = r"C:\Users\ibraz\Downloads\Campagnes Plateau G-Net Track Pro\drive_test"
...
# 🔹 Changer le répertoire courant vers le dossier des scripts
...
subprocess.run(["python", "plot_event.py"], check=True)  # ⚡ Nom corrigé ici
...
radio_stats_file = os.path.join(GRAPH_FOLDER, "resume_stats_radio.csv")
df_radio = pd.read_csv(radio_stats_file, index_col=0)

# Charger statistiques data
data_stats_file = os.path.join(GRAPH_FOLDER, "resume_stats_datatest.csv")
df_data = pd.read_csv(data_stats_file, index_col=0)

# Charger statistiques événements
events_stats_file = os.path.join(PROCESSED_FOLDER, "resume_stats_events.csv")
df_events = pd.read_csv(events_stats_file, index_col=0)
